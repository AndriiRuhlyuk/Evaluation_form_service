...
## Підсумок виконання (2026-08-25)

Кроки 1-7 виконані. Крок 8 (`drf-api-guard`, `django-deploy-checklist`) не починався.
Коміти в `develop`: `55f1a88` переїзд, `8b1a01e` `.superpowers` у `IGNORE_DIRS`,
`09a056e` реєстр знахідок. Плагін: `django-guardrails@evalforms-team-marketplace`, `1.0.0`.

### Scope установки - не за замовчуванням

`claude plugin install` кладе плагін у **user** scope, тобто вмикає його у *всіх* проєктах на
машині. Для гейтів, прив'язаних до Django-структури (`services.py`, `.env`), це розширення
периметра без потреби. Правильний стан - **project**: перевіряється рядком `Scope: project`
у `claude plugin list`, і `django-guardrails` не має зʼявлятись у `enabledPlugins` файлу
`~/.claude/settings.json`. Якщо зʼявився - плагін оголошений двічі й кожен хук спрацює двічі.

### Що НЕ доводить, що гейти живі

Три пастки, кожна коштувала окремої перевірки:

1. **`hooks-matrix` не доводить реєстрацію.** Вона годує скрипти payload'ом через stdin, тобто
   перевіряє їхню справність у відриві від сесії. Скрипт може бути бездоганним, а хук - не
   підключеним. 43 PASS означає «код цілий», не «гейт стоїть».
2. **Тест «запис у `.env` має блокуватись» не перевіряє хук.** У цьому репо раніше спрацьовує
   `deny`-правило `Read(**/.env.*)` зі `settings.json`, і запис не відбувається з зовсім іншої
   причини. Помилка виглядає так само. Придатні цілі: секрето-подібний літерал у `.py`
   (ловить `protect-secrets`) і `group_send` у будь-якому `services.py` (ловить
   `guard-layering`) - обидва мусять дати exit 2 **рівно по одному разу**; два повідомлення
   про той самий запис означають подвійну реєстрацію.
3. **Неблокувальні хуки нерозрізненно мовчать.** `format-python`, `guard-migrations`,
   `check-new-migrations` не дають exit 2, тож «не спрацював» і «спрацював, нічого не сказав»
   ззовні однакові. Єдина пряма перевірка - записати `.py` із поганим форматуванням і
   подивитись, чи `black` його переписав.

### Документація гниє там, де немає структури

Місць, що стали неправдою після переїзду, виявилось шість. Хук `check-layout-drift.sh` спіймав
**одне** - дерево в `## Project Layout`, бо тільки воно звіряється з `ls`. Решта п'ять були
прозою зі шляхом усередині: два абзаци в `## Commands`, `.claude/rules/data/migrations.md`,
хвіст запису `ARCH-3` у `Features_list.json` (вказував на `.claude/hooks/test-hooks.sh`, який
поїхав у плагін) і статусний абзац у робочому журналі.

Звідси процедура, дешевша за дисципліну: **після видалення або переїзду будь-якого шляху
грепнути репозиторій по його імені** - включно з `.claude/rules/`, реєстрами й планами, які
ніхто не вважає документацією. Автоматично ловиться лише те, що має машинно звірювану
структуру; усе інше треба шукати руками, і єдиний дешевий спосіб - греп по імені.

### Обмеження середовища, знайдені по дорозі

- Будь-яка команда `claude plugin ...` пише в `~/.claude/plugins/` і падає з `EPERM` -
  потрібен запуск поза пісочницею.
- `black .` обходить дерево з `stat` на кожному файлі й спотикається об `.env`, закритий
  `deny`-правилом. Виглядає як збій пісочниці, насправді - захист, що працює.
- У конвеєрі `cmd | tail` змінна `$?` показує код `tail`, а не `cmd`. Перший прогін `black`
  через це відзвітував `exit 0` під стектрейсом `PermissionError`. Правильно - `${PIPESTATUS[0]}`.

### Свіжий клон: оголосити ≠ встановити

Перевірено по документації Claude Code і підтверджено спостереженням у цій же сесії.
Закомічений `.claude/settings.json` дає **менше**, ніж здавалось при плануванні:

- **маркетплейс** реєструється автоматично, щойно користувач довіряє папці, без окремого
  запиту;
- **плагін** із зовнішнього джерела (GitHub) при цьому **не встановлюється**. Claude Code
  показує його як not installed і друкує команду `claude plugin install` - і все.

Емпіричне підтвердження було під рукою з самого початку, але його не розпізнали: на першому
ж виклику `claude plugin marketplace list` запис `evalforms-team-marketplace` уже був у
списку, хоча його ніхто не додавав - він прийшов із закоміченого `settings.json`. А в
`claude plugin list` стояв лише старий локальний плагін. Тобто обидві половини факту
спостерігались одночасно й були прийняті за хід плану.

Наслідок для команди: свіжий клон **без гейтів і без помилки** - стан не відрізняється від
здорового. Онбординг мусить містити явний крок `claude plugin install ... --scope project`
і перевірку `claude plugin list` (`Scope: project`, версія збігається з тегом).
