...
#
# F4, попередження для того, хто розширює ці два списки: позиційні токени
# `git log` НЕ прив'язані до кореня репозиторію (`_SAFE_POSITIONAL` нижче
# приймає будь-який шлях із вузького набору символів). Це безпечно РІВНО
# доти, доки жодна опція, що друкує ВМІСТ файлу (`-p`, `-L`, `-O<orderfile>`
# і подібні), не потрапить сюди - разом із такою опцією позиційний токен
# стає способом прочитати довільний файл під виглядом "git log". Додаєте
# опцію - спершу спитайте, чи вона щось друкує з файлу. Розгорнуто:
# README.md, розділ "Відомі межі".
_ALLOWED_EXACT_OPTS = {
    "--oneline",
    "--no-merges",
    "--name-only",
...
async def pre_tool_use_hook(input_data, tool_use_id, context) -> dict:
    ...
    Фінальний раунд ревʼю (minor): `DECISIONS.append` стояв ПОЗА будь-яким
    `try`, тому виняток на цьому кроці (зламаний `.append`, `__repr__`
    інструментального input'у, що піднімає) вибивав із hook, і SDK лишалось
    БЕЗ рішення - рівно те, що інваріант докстрінга ("будь-який збій на
    будь-якому кроці стає deny") обіцяє не допустити. Тепер запис у журнал
    має ВЛАСНИЙ `try/except`, окремий від першого: перший мусить закінчитись
    ДО запису, щоб журнал бачив фінальне `allowed` (див. наступний абзац), а
    другий перетворює збій журналу на deny.
    ...
    # Другий try, а не розширення першого: запис у журнал мусить бачити вже
    # ФІНАЛЬНЕ `allowed` (інакше повертається дефект round 3 - у журналі
    # ALLOW, у SDK deny), і водночас САМ не має права вибити виняток із
    # hook (дефект, який ловить фінальний раунд ревʼю: `.append` і
    # `repr(tool_input)` обидва можуть впасти, а докстрінг обіцяє "будь-який
    # збій на будь-якому кроці стає deny"). Збій журналу перемикає рішення
    # на deny: журнал - єдиний доказ, що пісочниця працювала, і дозвіл, який
    # неможливо задокументувати, не видається.
    try:
        DECISIONS.append((tool_name, repr(tool_input), "ALLOW" if allowed else "DENY"))
    except Exception as exc:
        allowed, reason = False, f"журнал рішень охоронця впав з винятком: {exc!r}"
        try:
            print(f"[guard] DENY  {tool_name} - {reason}", file=sys.stderr)
        except Exception:  # stderr теж зламаний - рішення вже прийняте
            pass
