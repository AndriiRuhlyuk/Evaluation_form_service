...
# поза сервісами.
#
# Інваріант: "services never broadcast" (.claude/rules/domain/services.md).
...
#
# Чому matcher + if: matcher звужує до трьох інструментів запису (ХТО),
# if - до шляху **/services.py (ЩО САМЕ). Без if процес хука стартував би на
# кожен Edit у репозиторії. Скрипт додатково перевіряє шлях сам, тому якщо if
# колись перестане зіставлятись, гейт деградує в "зайвий запуск", а не в
# хибне блокування.
import json
import os
import re
...
FEEDBACK = (
    ...
    "Інваріант проєкту - сервіси не роблять broadcast "
    "(.claude/rules/domain/services.md).\n"
    ...
    "виклич у working_form/views.py після виходу з atomic-блоку - там, де вже "
    "живе решта розсилок.\n"
)
