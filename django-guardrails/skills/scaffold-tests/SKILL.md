---
name: scaffold-tests
description: Use when adding or extending tests in a Django/DRF repo - writing a tests file for an app, covering services.py, permissions.py or the WebSocket consumer, raising coverage, or answering "are these tests actually any good". Scaffolds the file under the repo's conventions (APITestCase, literal slug URLs, status plus DB-row assertions, all_objects for soft delete), then proves the tests bite by running an AST-based mutation check against the module under test, and updates the project's test-coverage registry. Trigger whenever the user asks for tests, more coverage, a test review, or names an app or module to cover - even when they never say the word "scaffold".
argument-hint: '[app-or-module-path, e.g. question or project/views.py]'
allowed-tools: Read, Grep, Glob, Write, Edit, TodoWrite, Task, Bash(python3:*), Bash(uv run:*), Bash(bash:*), Bash(docker-compose:*), Bash(.venv/bin/black:*), Bash(.venv/bin/flake8:*), Bash(git:*), Bash(grep:*), Bash(sort:*), Bash(tr:*), Bash(echo:*)
---
...
the target project's coverage registry `.claude/rules/testing.md`), which is why the mutation check below is not optional.
...
If the four lines above show real values, they were resolved once before you started reading:
stale text, not commands you can re-run - call the tool yourself if you need fresher numbers.
If any of them still shows a literal `$ARGUMENTS` or a backticked command, this skill was
loaded somewhere that does not pre-expand them; run those same commands with Bash before
going further. Never treat the inventory as live either way.
...
- [ ] **Step 1: Probe the environment.** Run exactly:
  ...
  ```bash
  python3 ${CLAUDE_PLUGIN_ROOT}/skills/scaffold-tests/scripts/preflight.py --app <app> --target <module> --format text
  ```
...
- [ ] **Step 3: Load the conventions.** Read `${CLAUDE_PLUGIN_ROOT}/skills/scaffold-tests/references/conventions.md`
  now. Paths into this skill are written from `${CLAUDE_PLUGIN_ROOT}` because the plugin does
  not live in your working directory - a bare `references/conventions.md` resolves to nothing
  there, and so does `.claude/skills/...`. Paths into the code under test stay relative to the
  repo root, which IS your working directory. It holds the
...
- [ ] **Step 7: Prove the tests bite.** Run exactly:
  ...
  ```bash
  bash ${CLAUDE_PLUGIN_ROOT}/skills/scaffold-tests/scripts/verify-tests.sh --app <app> --target <module> --format json
  ```
...
- [ ] **Step 8: Validation loop.** While `survived` is non-empty: read
  `${CLAUDE_PLUGIN_ROOT}/skills/scaffold-tests/references/mutation-guide.md`, decide per survivor (write a test / accept the gap with a
...
- [ ] **Step 10: Update the registry.** Add the new file to the "The real baseline" section of
  the target project's coverage registry (`.claude/rules/testing.md`) with one line saying
  what it covers. A registry that has drifted from reality is worse than no registry, because
  it is trusted. If the project has no such file, do not create one uninvited: say in the
  report that coverage was not recorded anywhere, and let the user decide where it belongs.

- [ ] **Step 11: Report, do not commit.** Fill in `${CLAUDE_PLUGIN_ROOT}/skills/scaffold-tests/templates/test-report.md` section by
...
Paths are written from `${CLAUDE_PLUGIN_ROOT}`, not from your working directory: the plugin
lives outside the repo under test, so a bare `references/conventions.md` resolves to nothing.

| Path | Open it when |
|---|---|
| `${CLAUDE_PLUGIN_ROOT}/skills/scaffold-tests/references/conventions.md` | step 3, before the first assertion |
| `${CLAUDE_PLUGIN_ROOT}/skills/scaffold-tests/references/mutation-guide.md` | step 8, or whenever a survivor needs a decision |
| `${CLAUDE_PLUGIN_ROOT}/skills/scaffold-tests/templates/test-report.md` | step 11, to shape the final answer |
| `${CLAUDE_PLUGIN_ROOT}/skills/scaffold-tests/scripts/preflight.py` | run at step 1; read only if its output is confusing |
| `${CLAUDE_PLUGIN_ROOT}/skills/scaffold-tests/scripts/verify-tests.sh` | run at step 7; read only to change its contract |
| `${CLAUDE_PLUGIN_ROOT}/skills/scaffold-tests/scripts/mutate.py` | called by `verify-tests.sh`; run directly only with `--dry-run` |
